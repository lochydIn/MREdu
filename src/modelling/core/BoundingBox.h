...
struct BoundingBox {
    ...
    // 3 constructors
    BoundingBox() : min(1e30f), max(-1e30f) {} // Empty Box.
    BoundingBox(const glm::vec3& p) : min(p), max(p) {} // Point-sized box.
    BoundingBox(const glm::vec3& min, const glm::vec3& max) : min(min), max(max) {} //Full box.
