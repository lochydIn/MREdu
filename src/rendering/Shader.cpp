...
{
    const float r = glm::max(roughness, 0.001f);
                       ...
                       const glm::vec3& N, const glm::vec3& V, const glm::vec3& L)
{
    ...
    const glm::vec3 F = fresnelSchlick(surfaceColour, metallic, reflective, nDotV);
    const float G = geometryShading(nDotL, nDotV, r);
    ...
    {
        D = distributionGGXIsotropic(nDotH, r);
    }
            ...
            if (params.softShadows && params.shadowSamples > 1 && pointLight->getRadius() > 0)
            {
                ...
                for (int i = 0; i < params.shadowSamples; i++)
                {
                    ...
                    if (nDotL > 0.0f)
                    {
                        ...
                        {
                            glm::vec3 brdfValue = bRDF(surfaceColour, roughness, metallic, reflectivity,
                                                       clearcoat, clearcoatRoughness,
                                                       sheen, sheenColour, anisotropy, anisotropyRotation, T, B, N, V,
                                                       L);
